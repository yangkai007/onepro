...
export default {
  ...
  computed:{
    // cartFoods 总价格总数量  info 起送价 及各种信息
    // // 在购物车中获取到cartFoods的state 以及商家的info
    ...mapState(['cartFoods','info']),  // 定义在store的getters。js中
    // // 获取相应的Getters里的数据
    ...mapGetters(['totalCount', 'totalPrice']), //总价格 总数量 
    payClass () {
    ...
    isShow: function () {
      if (this.isShow) {
        this.$nextTick(() => {
          // 实现BScroll的实例是一个单例
          if (!this.scroll) {
            this.scroll = new BScroll('.list-content', { //需要滚动的列表类名
              click: true
            })
          } else {
